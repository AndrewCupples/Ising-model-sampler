...
def update_edge(edges, random_variables, graph_top, graph_bottom, vertex_list, uniform_random_variables):
    top_copy = graph_top.copy()
    bottom_copy = graph_bottom.copy()
    update_probability = rcm_p
    components_top = connected_components(top_copy, vertex_list)
    components_bottom = connected_components(bottom_copy, vertex_list)
    for y in range(len(uniform_random_variables)):
        rv = random_variables[-y-1]
        edge_choice = edges[-y-1]
        choice_top = glauber_func(rv, edge_choice, top_copy, update_probability, components_top, vertex_list)
        choice_bottom = glauber_func(rv, edge_choice, bottom_copy, update_probability, components_bottom, vertex_list)
        if choice_top:
            top_copy = add_edge(top_copy, edge_choice)
            components_top = components_updates(components_top, edge_choice, "remove", top_copy, vertex_list)
        if choice_bottom:
            bottom_copy = add_edge(bottom_copy, edge_choice)
            components_bottom = components_updates(components_bottom, edge_choice, "remove", bottom_copy, vertex_list)
        if not choice_top:
...
def glauber_func(random_variable, edge, graph, prob, connected_comp, vertex_list):
    # part1 = rcm_p / (1 - rcm_p)
    ...
    if edge_in(graph, edge):
        update_graph = remove_edge(graph, edge)
        # connected_joined = len(connected_components(update_graph))
        new_connected = components_updates(connected_base, edge, "remove", update_graph, vertex_list)
        connected_joined = len(new_connected)
    else:
        update_graph = add_edge(graph, edge)
        # connected_joined = len(connected_components(update_graph))
        new_connected = components_updates(connected_base, edge, "add", update_graph, vertex_list)
        connected_joined = len(new_connected)
...
def connected_components(graph, vertex_list):
...
def components_updates(components_use, edge, update, graph, vertex_list):
    (vertex1, vertex2) = edge
    new_components = []
    if update == "add":
        comp_1 = 0
...
def run_experiment(row_number):
    ...
    while not converge:
        ...
        top_graph, bottom_graph = update_edge(edges_used, uniform_random_variables,
                                              top_state, bottom_state, vertex_list, uniform_random_variables)

        converge = convergence(top_graph, bottom_graph)
        if not converge:
            time *= 2
    return top_graph, bottom_graph, time


def print_output(row_number, top_graph, vertex_list):
    # convert sample to Ising and output
    components = connected_components(top_graph, vertex_list)
    spins = ising_rng.integers(0, 1, len(components), endpoint=True)
    # output = np.zeros((row_number, row_number, row_number), int)
    output = np.zeros((row_number, row_number), int)
    for x in range(len(components)):
        cc = components[x]
        for t in range(len(cc)):
            # output[cc[t]//row_number][cc[t] % row_number] = spins[x]
            output[cc[t] // row_number // row_number][(cc[t] // row_number)
                                                      % row_number][cc[t] % row_number] = spins[x]
    print(output)
    print(top_graph)
